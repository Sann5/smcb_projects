...
Use the function cor.test() to test the null hypothesis of no correlation between A and B as it gives us a p-value of p-value = 0.8401. What is your conclusion?
...
qplot(residualA_C,residualB_C)+theme_classic()+geom_abline(intercept =slr.coef[1], slope = slr.coef[2],colour="blue" )+ ggtitle("Partial Correlation ")
...
With the p-value of 6.6e-13 we can reject the Null Hypothesis which states that there is no partial correlation. We can say that A and B are conditionally independent given C.
...
Yes the PC algorithm has successfully estimated the correct graph when α =0.01. The arrow direction of B & E can not be estimated as it is indistinguishable. The arrow cannot be oriented.
