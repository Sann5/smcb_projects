...
# Apply thinning by taking every 5th sample
dd_thin <- dd[c(TRUE,rep(FALSE,4)), ]
...
$$
P(R=T|S=T,W=T) = \sum_c P(R=T,C|S=T,W=T)=0.33\\
P(R=F|S=T,W=T) = \sum_c P(R=F,C|S=T,W=T)=0.67
$$
...
=
\frac
{P (W = T | R = T, S = T) P(R = T | S = T) P(S = T)}
{\sum_{c \in \{F,T\}} P(C = c) P(S = T, W = T | C = c)}
$$
...
\frac
{P (W = T | R = T, S = T) P(R = T | S = T) P(S = T)}
{\sum_{c \in \{F,T\}} P(C = c) P(W = T | S = T, C = c) P(S = T | C = c)}
$$

Here we compute the quantities we dont know inmediatly from the table:
$$
P(R = T | S = T) =  \sum_{c \in \{F,T\}} P(C=c) P(R = T | S = T, C = c) 
$$

$$
=  \sum_{c \in \{F,T\}} P(C=c) P(R = T |  C = c) =  P(R = T) = 0.5 \times 0.8 + 0.5 \times 0.2 = 0.5
$$
...
$$
P(S = T) = \sum_{c \in \{F,T\}} P(C=c) P( S = T | C = c) = 0.5 \times 0.1 + 0.5 \times 0.5 = 0.3
$$

$$
P(W = T | S = T, C = T) = \sum_{r \in \{F,T\}} P(R=r) P(W = T | S = T, R = r, C = T)
$$

$$
= \sum_{r \in \{F,T\}} P(R=r) P(W = T | S = T, R = r) = 0.5 \times 0.99 + 0.5 \times 0.9 = 0.945
$$
...
\frac
{P (W = T | R = T, S = T) P(R = T | S = T) P(S = T)}
{\sum_{c \in \{F,T\}} P(C = c) P(W = T | S = T) P(S = T | C = c)}
$$

$$
= \frac
{0.99 \times 0.5 \times 0.3}
{0.5 \times 0.945 \times 0.1 + 0.5 \times 0.945 \times 0.5} = 0.5238095
$$

Compared with (c) and (h) the analytically computed probability seems quite different.
