...
To recover the least squared estimator in the Bayesian setting one seeks to maximize the probability of the labels $y_{1:n}$ given their corresponding feature vectors $x_{1:n}$ and the parameters of the model. In this case since we are speaking of linear regression, our model is given by $Y = h(X) + \epsilon$, where $\epsilon \sim \mathcal{N}(0, \sigma^2)$ and $h(X) = w^TX$, therefore our model parameters are $\sigma^2$ and $w$ (note that we use $w$ instead of $\beta$, like in the homework description for the weights). Then we seek to maximize this probability by finding the best $w$, since we $\sigma^2$ is assumed to be fixed. 

$$
\hat{w} = \underset{w}{\operatorname{argmax}} P(y_{1:n} | x_{1:n}, w, \sigma^2) 
$$

Assuming that all $n$ data points are $i.i.d$ and by applying the log, we can rewrite this expression as follows.

$$
= \underset{w}{\operatorname{argmin}} - \sum_{i=1}^n \log P(y_{i} | x_{i}, w, \sigma^2) 
$$

Then we can plug in our assumptions about the model to give way to the optimal set of weights $\hat{w}$.
...
Coming back to our optimization problem we can now write it as follows.
...
Now that we have an expression for the likelihood $P(y_{1:n} | x_{1:n}, w, \sigma^2)$ we would like to perform MAP estimation on the probability of the weights given the data $P(w | y_{1:n}, x_{1:n})$, since maximizing this probability is equivalent to finding the most probable weights $\hat{w}$. 
...
By maximizing this quantity and plugging in our derived expression for the likelihood and a Laplassian prior, we obtain the Lasso regression estimator. 
