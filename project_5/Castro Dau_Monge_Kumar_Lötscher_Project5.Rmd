...
$$
Because the terms inside the summations concerning $Z_1$ and $Z_2$ only depend on $Z_3$ we only need to calculate them once per $Z_3, Z_1$ and $Z_3, Z_2$ value pairs ($4(4+4) = 32$). Then we need to compute $P(Z_{3}|Z_{4})$ 4 times, once for each per $Z_3$ value pair. Finally we need to compute $P(Z_{4})P(X_{5}|Z_{4})$ 4 times, once for each value $Z_4$ In total we compute $62$ terms.
...
```{r}
likelihood_optimised_rate_only = optim.pml(likelihood, optQ = TRUE, optEdge = FALSE, model='K80')
