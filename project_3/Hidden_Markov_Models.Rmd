...
Given that $Z_{n-1} = k$, there are $K$ transition probabilities into state $Z_{n}$, one for each value that $Z_n$ can take. Since $\sum_{k'=1}^K P(Z_n = k' \mid  Z_{n-1} = k) = 1$ we only need to define $K-1$ transition probabilities for each possible imputation of $Z_{n-1} = k$. Hence we need to define $K*(K-1)$ transition probabilities. We are however assuming that these transition probabilities are all the same for all $(Z_n, Z_{n-1})$ pairs. 

Similarly because $\sum_{m=1}^M P(X_n = m \mid Z_n = k) = 1$, and $Z_{n}$ can take on $K$ different values, we need to define overall $K*(M-1)$ emission probabilities (again asssuming that these are site independent). 
...
Alternatively one could compute a very high power of $T^T$ (e.g. $(T^T)^n$) and obtain a matrix which columns contain the stationary distribution. An easy way to reason about this is that any matrix that is decomposable can be written as $A = S \Lambda S^{-1}$, where $S$ is the matrix of eigenvectors and $\Lambda$ is a diagonal matrix with the eigenvalues. One can show that the powers of such a matrix can be written as $A^n = S \Lambda^n S^{-1}$. Since $\Lambda$ is diagonal $\Lambda^n$ is also diagonal but with entries $\lambda_{ij}^n$, which are the eigenvalues to the power of $n$. Since we are dealing with a Markov matrix all other eigenvalues except the one equal to 1 will go to 0 as $n$ goes to infinity. This in turn will yield a matrix where the columns contain the stationary distribution.
...
I.mean<- apply(simplify2array(Ilist),c(1,2),mean)
I.sd<- apply(simplify2array(Ilist),c(1,2),sd)
...
for (i in 1:(nrow(PTest_data))){
  sampled=sample(unique.ss, nchar(Test_data$KnownPath[i]),replace = TRUE) #ramdom sampling seq--> ADD WEIGHTS/PROBS
  given = unlist(strsplit(PTest_data$KnownPath[i],""))
