...
We know that $\sum_{k}P(C_k | D_i) = 1$ and therefore that $P(C_A | D_i)  = 1 - P(C_B | D_i)$ for observation $i$. We also know that $P(D_i | \theta) \sim Binom(10, \theta) = {10 \choose k} \theta^k (1-\theta)^{10-k}$, where $k$ is the number of heads in observation $D_i$.

Given that $P(C_A) = 0.6$ and $P(C_B) = 0.4$, and that $\theta_A$ and $\theta_A$ are 0.7 and 0.4 respectively; for the first observation $D_1$ we have that $k = 4$ and therefore:

$$
P(C_A | D_1) = \frac{0.6*{10 \choose 4} * 0.7^4 * 0.3^6}{0.6*{10 \choose 4} * 0.7^4 * 0.3^6 + 0.4*{10 \choose 4} * 0.4^4 * 0.6^6} \approx 0.18 \\
...
$$
P(C_A | D_1) = \frac{0.6*{10 \choose 8} * 0.7^8 * 0.3^2}{0.6*{10 \choose 8} * 0.7^8 * 0.3^2 + 0.4*{10 \choose 8} * 0.4^8 * 0.6^2} \approx 0.97 \\
...
coin_EM <- function(D, k, lambda, theta, tolerance = 1e-2) {
  ...
  while (diff > tolerance) {
    ...
    P <- rbind(dbinom(apply(D, 1, sum), 100, theta[1]), 
               dbinom(apply(D, 1, sum), 100, theta[2]))
    ll_hid = sum(gamma * log(lambda*P))
    
    # Update observed log-likelihood
    ## TODO:
    ll_obs = ll_hid - entropy + kl_divergence
    ...
    lambda = apply(gamma, 1, sum) / N
    
    # Recompute probability of heads for each coin
    ## idea for this expression: http://karlrosaen.com/ml/notebooks/em-coin-flips/
    
    theta[1] = sum(gamma[1, ] * apply(D, 1, sum)) / 
               sum(gamma[1, ] * 100)
    theta[2] = sum(gamma[2, ] * apply(D, 1, sum)) / 
               sum(gamma[2, ] * 100)
